...
describe("buildAnthropicBillingHeader version suffix edge cases", () => {
    ...
    it("handles empty message content without crashing", () => {
        delete process.env.CLAUDE_CODE_ATTRIBUTION_HEADER;
        expect(() => buildAnthropicBillingHeader("2.1.107", [{ role: "user", content: "" }])).not.toThrow();
    });

    it("handles single-character message", () => {
        delete process.env.CLAUDE_CODE_ATTRIBUTION_HEADER;
        expect(() => buildAnthropicBillingHeader("2.1.107", [{ role: "user", content: "x" }])).not.toThrow();
    });

    it("handles empty messages array", () => {
        delete process.env.CLAUDE_CODE_ATTRIBUTION_HEADER;
        expect(() => buildAnthropicBillingHeader("2.1.107", [])).not.toThrow();
    });

    it("includes the CC version in the header output", () => {
        delete process.env.CLAUDE_CODE_ATTRIBUTION_HEADER;
        const header = buildAnthropicBillingHeader("2.1.107", [{ role: "user", content: "hello world" }]);
        expect(header).toContain("2.1.107");
    });
});
