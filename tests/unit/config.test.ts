...
describe("DEFAULT_CONFIG", () => {
    ...
    it("uses the default signature profile", () => {
...
describe("loadConfig", () => {
    ...
    it("merges a valid signature profile from config file", () => {
        mockExistsSync.mockReturnValue(true);
        mockReadFileSync.mockReturnValue(JSON.stringify({ signature_profile: DEFAULT_SIGNATURE_PROFILE_ID }));
        const config = loadConfig();
        expect(config.signature_profile).toBe(DEFAULT_SIGNATURE_PROFILE_ID);
    });

    it("rejects an unknown signature profile from config file", () => {
        mockExistsSync.mockReturnValue(true);
        mockReadFileSync.mockReturnValue(JSON.stringify({ signature_profile: "cc-does-not-exist" }));
        const config = loadConfig();
        expect(config.signature_profile).toBe(DEFAULT_SIGNATURE_PROFILE_ID);
    });
