...
import { calculateRetryDelay, fetchWithRetry, shouldRetryStatus } from "../../../src/request/retry.js";

function makeResponse(status: number, headers?: ResponseInit["headers"]): Response {
    ...
    it("retries thrown retryable network errors and marks the next attempt as fresh-connection", async () => {
        ...
        const doFetch = vi.fn(async ({ forceFreshConnection = false }: { forceFreshConnection?: boolean } = {}) => {
            ...
            if (forceFreshConnectionByAttempt.length === 1) {
                throw Object.assign(new Error("Connection reset by server"), {
                    code: "ECONNRESET",
                });
            }
