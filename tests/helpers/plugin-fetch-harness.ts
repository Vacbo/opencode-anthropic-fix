...
export interface FetchHarness {
    /** Plugin instance, including hook entry points */
...
export async function createFetchHarness(opts: HarnessOptions = {}): Promise<FetchHarness> {
    ...
    const { AnthropicAuthPlugin } = await import("../../src/index.js");
    const { loadConfig, loadConfigFresh } = await import("../../src/config.js");
    const { loadAccounts } = await import("../../src/storage.js");
    ...
    return {
        plugin: plugin as Record<string, unknown>,
        fetch: result.fetch,
        mockFetch,
        tearDown,
        waitFor,
